...
sett = Settings('https://tables.mws.ru/fusion/v1/datasheets/dstcwabw4D8JhP1VwL', 'uskPUFZhMwASVADEGwgI4XN')
...
async def main(url_list):
    ...
    for i in range(3):
        if len(posts_yt) > i:
            comments_tg = await telegram_parser.get_comments_for_post(posts_yt[i]['uri'])
        if len(posts_vk) > i:
