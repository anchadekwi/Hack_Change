...
class TgClient:
    ...
    async def authenticate(self):
        try:
            ...
            if "t.me/" in self.channel_uri:
                username = (
                    self.channel_uri.split("t.me/")[-1].split("?")[0].split("/")[0]
                )
            ...
            if not hasattr(entity, "username") or entity.username is None:
                print(
                    f"У канала {username} нет публичного username — возможно, это приватный чат"
                )
                return False

            if entity.username.lower() != username.lower():
                print(
                    f"ОШИБКА: найден другой канал! Ожидался: {username}, найден: {entity.username}"
                )
                return False
    ...
    async def get_comments_for_post(self, post_uri: str) -> list[dict]:
        ...
        try:
            ...
            if (
                not hasattr(message, "replies")
                or not message.replies
                or message.replies.replies == 0
            ):
                return []

            comments = []
            async for comment in self.client.iter_messages(
                entity=self.channel, reply_to=message_id, limit=500
            ):
